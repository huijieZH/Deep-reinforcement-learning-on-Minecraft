...
class DQN(MineCraftRL):
    ...
    def step_train(self, step, epsilon):
        ...
        # DQN
        Y_minibatch = R_batch.reshape((-1, 1)) + self.args.gamma * torch.max(self.target_Qnet(St1_batch), axis = 1)[0].reshape((-1, 1))
...
class DoubleDQN(DQN):
    ...
    def step_train(self, step, epsilon):
        ...
        Y_minibatch = R_batch.reshape((-1, 1)) + self.args.gamma * torch.sum(A_Y_minibatch * self.target_Qnet(St1_batch), axis = 1, keepdim = True)
